...
const TOKEN = "MTQ4MzQyMjEzODI0NDM5OTE2NQ.GY6BnP.YmoSz4Pm17yeaGkSQbbBXBo1dZ95NVzyX-BJEY";
const GUILD_ID = "1480125656753766554";
const WELCOME_CHANNEL = "1480128982996357313"; // ايدي قناة الترحيب
const AUTO_ROLE_ID = "1480937552386064536"; // ايدي الرتبة التلقائية
...
📅 <t:${Math.floor(Date.now()/1000)}:F>`);
    ...
    if (welcomeChannel) welcomeChannel.send(`👋 أهلاً وسهلاً ${member} في السيرفر ❤️`);

    const role = member.guild.roles.cache.get(AUTO_ROLE_ID);
    if (role && !member.roles.cache.has(role.id)) {
        try {
            await member.roles.add(role);
            sendLog(channel, `🎁 تم اعطاء رتبة <@&${role.id}> للعضو ${member}`);
        } catch (err) {
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
    ...
    if (added.size || removed.size) {
        ...
        added.forEach(role => {
            sendLog(roleChannel, `🎭 **إعطاء رتبة**
👤 ${newMember}
🎖️ ${role}
🛠️ بواسطة: ${executor}`);
        });
...
client.on("interactionCreate", async i => {
    ...
    if (i.commandName === "clear") {
        ...
        if (amount.toLowerCase() === "all") {
            ...
            i.reply({ content: `🧹 تم مسح جميع الرسائل في الروم`, ephemeral: true });
        } else {
            ...
            i.reply({ content: `🧹 تم مسح ${num} رسالة`, ephemeral: true });
        }
    }
});
